...
import { GITMOJI_SCREEN_SIZE, T_SCREEN_SIZE_TYPE } from '../../ipc/screen'
import { getCenterPositionBoundByRouter } from '../../utils/display'

const __dirname = path.dirname(fileURLToPath(import.meta.url))

class GitmojiWindow implements TWindows {
	public instance: BrowserWindow | null = null

	routerName: T_SCREEN_SIZE_TYPE = 'GITMOJI'
	...
	create(onLoad?: () => void) {
		...
		this.instance = new BrowserWindow({
			...
			show: false,
			fullscreenable: true,
			width: GITMOJI_SCREEN_SIZE.width,
			height: GITMOJI_SCREEN_SIZE.height,
			x,
			y,
		})
...
export const gitmojiWindow = new GitmojiWindow()
